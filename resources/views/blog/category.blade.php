{{--
    Category archive — lists the published posts in one category.
    Fully editable: this is a plain Blade view. $category is the current
    category, $posts is a paginator of its published posts.
--}}
@extends('rjcms::layouts.app')

@section('title', $category->name . ' · ' . setting('site.name', config('app.name')))

@section('body')
<div class="mx-auto max-w-3xl px-4 py-12 sm:px-6">
    <p class="text-sm text-gray-500">
        <a href="{{ route('blog.index') }}" class="hover:underline">Blog</a> · Category
    </p>
    <h1 class="mt-1 text-3xl font-bold tracking-tight text-gray-900">{{ $category->name }}</h1>
