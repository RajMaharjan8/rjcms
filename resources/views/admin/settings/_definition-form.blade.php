...
<form method="POST" action="{{ $action }}" class="max-w-3xl"
    ...
    <x-admin.postbox title="Setting Definition" body-class="px-5 py-1">
        <div class="divide-y divide-wp-border-light">
            <div class="grid gap-2 py-4 sm:grid-cols-[180px_minmax(0,1fr)] sm:items-center sm:gap-4">
                ...
                <div>
                    <input type="text" x-model="form.key" placeholder="site.title" class="{{ $inputClass }}">
                    <p class="mt-1 text-xs text-wp-muted">Used to read the value, e.g. <span class="font-mono">setting('<span x-text="form.key || 'site.title'"></span>')</span></p>
                </div>
            </div>
            <div class="grid gap-2 py-4 sm:grid-cols-[180px_minmax(0,1fr)] sm:items-center sm:gap-4">
                <label class="text-sm font-semibold text-wp-ink">Display name</label>
                <input type="text" x-model="form.display_name" placeholder="Site title" class="{{ $inputClass }}">
            </div>
            <div class="grid gap-2 py-4 sm:grid-cols-[180px_minmax(0,1fr)] sm:items-center sm:gap-4">
                <label class="text-sm font-semibold text-wp-ink">Group / category</label>
                <div>
                    <input type="text" x-model="form.group" list="setting-groups" placeholder="Site" class="{{ $inputClass }}">
        ...
        <x-slot:footer>
            <div class="flex items-center justify-end gap-3">
                <a href="{{ route('admin.settings.index') }}" class="text-sm font-medium text-wp-muted hover:text-wp-ink">Cancel</a>
